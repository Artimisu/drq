...
class DRQAgent(object):
    ...
    def update_critic(self, obs, obs_aug, action, reward, next_obs,
        ...
        #stn training
        noise = torch.randn(obs_aug.size(0), self.noise_dim)#.cuda()
        input_aug_obs, target_augQ, div_loss, diversity_loss = \
                self.aug_net(noise, obs_aug, target_Q, require_loss=True)
        action_comb = torch.cat([action, action], dim=0)
        stn_target_Q1, stn_target_Q2 = self.critic(input_aug_obs, action_comb)
        stn_target_loss = F.mse_loss(stn_target_Q1, target_augQ)+F.mse_loss(stn_target_Q1, target_augQ)
